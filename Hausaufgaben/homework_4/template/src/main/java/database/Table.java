...
public class Table<T> implements Iterable<T> {
     ...
     */
    public <R> R reduce(Aggregation<T, R> aggregation) { // Das erste <R> bedeutet, dass der Rückgabetyp
        // von reduce nicht zwingend das gleiche sein muss, wie das, was in die Methode gegeben / übergeben
        // wurde
        R curr = aggregation.initial();
        for (T currListEl: collection) {
